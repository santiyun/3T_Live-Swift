...
class TTTSCdnViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        h265 = TTManager.h265
        doubleChannel = TTManager.doubleChannel
        if h265 {
        ...
        let isCustom = TTManager.cdnCustom.isCustom
        refreshState(isCustom, profile: TTManager.cdnProfile)
        if isCustom {
            profileIndex = 7
            ...
            pickView.selectRow(Int(TTManager.cdnProfile.rawValue / 10), inComponent: 0, animated: true)
        }
    }
    ...
    public func saveAction() -> String? {
        if videoTitleTF.text == "自定义" {
            ...
            TTManager.cdnCustom = (true, CGSize(width: sizeW, height: sizeH),bitrate,fps)
        } else {
            let index = pickView.selectedRow(inComponent: 0)
            let profile = TTTRtcVideoProfile(rawValue: UInt(index * 10))!
            TTManager.cdnProfile = profile
            TTManager.cdnCustom = (false, TTManager.cdnProfile.mixSize(), profile.bitrate(), 15)
        }
        TTManager.h265 = h265
        TTManager.doubleChannel = doubleChannel
        return nil
    }
...
extension TTTSCdnViewController: UIPickerViewDelegate, UIPickerViewDataSource {
