...
class TTTSLocalViewController: UIViewController {
...
//        print("---local:\(view)")
        ...
        let isCustom = TTManager.localCustomProfile.isCustom
        refreshState(isCustom, profile: TTManager.localProfile)
        if isCustom {
            ...
            pickView.selectRow(Int(TTManager.localProfile.rawValue / 10), inComponent: 0, animated: true)
        }
    }
    ...
    public func saveAction() -> String? {
        ...
        if videoTitleTF.text == "自定义" {
            ...
            TTManager.localCustomProfile = (true,CGSize(width: sizeW, height: sizeH),bitrate,fps)
        } else {
            TTManager.localCustomProfile.isCustom = false
            let index = pickView.selectedRow(inComponent: 0)
            TTManager.localProfile = TTTRtcVideoProfile(rawValue: UInt(index * 10))!
        }
...
extension TTTSLocalViewController: UIPickerViewDelegate, UIPickerViewDataSource {
